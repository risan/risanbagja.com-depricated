...
class CleanScssBuildWebpackPlugin {
  static getEmittedJsFiles(assets) {
    return Object.entries(assets).reduce((emittedJsFiles, props) => {
      ...
      return CleanScssBuildWebpackPlugin.isJsFile(props[0]) ?
        emittedJsFiles.concat(props[0]) : emittedJsFiles;
    }, []);
  }
  ...
  static getJsFilesFromCssOrScssBuild(chunks) {
    ...
    return CleanScssBuildWebpackPlugin.getJsFilesFromChunks(chunksFromCssOrScss);
  }

  static isChunkFromCssOrScss(chunk) {
    const validExtensions = ['.css', '.scss'];

    const cssOrScssModules = chunk.getModules().filter(module =>
  ...
  apply(compiler) {
    compiler.plugin('after-emit', (compilation, callback) => {
      const emittedJsFiles = CleanScssBuildWebpackPlugin.getEmittedJsFiles(compilation.assets);
      ...
      const uneededJsFiles = CleanScssBuildWebpackPlugin.getJsFilesFromCssOrScssBuild(compilation.chunks);
